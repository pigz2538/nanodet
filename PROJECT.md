...
3. **Head（检测头）**：在融合后的特征图上预测分类（classification）与分布回归（distribution regression），典型实现为 NanoDetPlusHead，采用 Generalized Focal Loss 系列损失。针对目标 NPU 工具链的算子限制，本项目中对 NanoDetPlusHead 的 ONNX 导出路径做了调整：将分类与回归输出拆分为两个独立卷积，并将所有 stride 的输出以多个 4D 张量形式返回，避免导出 `Transpose`、`Slice` 与冗余 `Reshape`。
...
4. 构造随机输入，调用 `torch.onnx.export` 导出为 ONNX，输入名称为 `data`。针对 NPU 部署场景，检测头会输出每个 stride 一个 4D 张量（如 `cls_dis_stride_8`、`cls_dis_stride_16` 等），避免跨 stride 的拼接与转置。
